...
from dh.utils.config import Config, load_config
...
class ProjectContext:
    ...
    def require_project(self) -> tuple[ProjectType, Path]:
        ...
        if self.has_frontend and not self.has_backend:
            ...
            console.print("❌ Ambiguous context: workspace has both FE and BE projects", style="red")
            console.print("Run command from within a specific project directory", style="yellow")
            raise typer.Exit(1)
        else:
            console.print("❌ No project found in workspace", style="red")
            console.print("Expected FE markers: package.json + next.config.ts", style="yellow")
            console.print("Expected BE markers: pyproject.toml + main.py", style="yellow")
            raise typer.Exit(1)
